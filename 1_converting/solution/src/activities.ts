...
export async function assignReviewer(request: RFIRequest): Promise<RFIRequest & { assignedReviewer: string }> {
  ...
  // Simulate reviewer assignment (in production this would call an internal API)
...
export async function notifyStakeholders(
  ...
  // Simulate notification (in production this would send emails, Slack messages, etc.)
  return {
    rfiId: request.rfiId,
    status: 'pending',
